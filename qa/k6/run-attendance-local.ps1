...
foreach ($concurrency in @(10, 25, 50, 100)) {
  ...
  $env:ZENTIME_EMPLOYEE_OFFSET = [string]$offset
  $summaryFile = Join-Path $SummaryDir "attendance-$concurrency.json"
  Write-Host "Running attendance load test at concurrency $concurrency against $BaseUrl with employee offset $offset"
  & $k6Command run --summary-export $summaryFile qa/k6/attendance.load.js
  $offset += $concurrency
}
